...
Create a git-ignored `.env` file at the repo root so Compose can resolve `${POSTGRES_USER}`/`${POSTGRES_PASSWORD}`/`${POSTGRES_DB}` (this mirrors the same-named variables/secret stored in the `dev` GitHub Environment, used the same way by [deploy.yml](.github/workflows/deploy.yml) in CI):
...
| File | Kind | Purpose |
|---|---|---|
| [backend-deployment.yaml](k8s/backend-deployment.yaml) | `Deployment` | Runs the backend image; `POSTGRES_*` env vars from `backend-config`/`postgres-credentials`; readiness/liveness probes on `/health` |
| [backend-service.yaml](k8s/backend-service.yaml) | `Service` (`ClusterIP`) | Internal DNS name `backend:8000` |
| [backend-configmap.yaml](k8s/backend-configmap.yaml) | `ConfigMap` | Non-secret backend config: `POSTGRES_HOST`, `POSTGRES_PORT`, `POSTGRES_DB` |
| [backend-pvc.yaml](k8s/backend-pvc.yaml) | `PersistentVolumeClaim` | Left over from an earlier SQLite-based setup; no longer mounted by `backend-deployment.yaml` now that Postgres owns storage — kept only for reference |
| [frontend-deployment.yaml](k8s/frontend-deployment.yaml) | `Deployment` | Runs the frontend image; `API_URL` from `frontend-config`; probes on `/_stcore/health` |
| [frontend-service.yaml](k8s/frontend-service.yaml) | `Service` (`ClusterIP`) | Internal DNS name `frontend:8501` |
| [frontend-configmap.yaml](k8s/frontend-configmap.yaml) | `ConfigMap` | `API_URL: http://backend:8000` |
| [postgres-statefulset.yaml](k8s/postgres-statefulset.yaml) | `StatefulSet` | Runs `postgres:16` with a dedicated `PersistentVolumeClaim` per replica via `volumeClaimTemplates`; credentials from `postgres-credentials` |
| [postgres-service.yaml](k8s/postgres-service.yaml) | `Service` (headless, `clusterIP: None`) | Stable per-Pod DNS names required by the `StatefulSet` |
| [nginx-deployment.yaml](k8s/nginx-deployment.yaml) | `Deployment` | Runs `nginx:latest`, mounting `nginx-config` and the `nginx-tls` secret |
| [nginx-service.yaml](k8s/nginx-service.yaml) | `Service` (`LoadBalancer`) | Publishes `80`/`443` to the host — Docker Desktop binds this to `localhost` |
| [nginx-configmap.yaml](k8s/nginx-configmap.yaml) | `ConfigMap` | Same reverse-proxy/TLS logic as [nginx/nginx.conf](nginx/nginx.conf), adapted to reference Kubernetes `Service` DNS names/ports |
...
Apply everything (this order lets PVCs/Secrets exist before the Pods that reference them, though `kubectl apply -f k8s/` for the whole folder at once also works — Kubernetes retries scheduling until dependencies appear):
...
Test-only dependencies (`pytest`, `httpx`) are tracked alongside production dependencies in [backend/requirements.txt](backend/requirements.txt), so a single install covers both.
...
|---|---|---|
| **Auto Tests** ([auto_tests.yml](.github/workflows/auto_tests.yml)) | Push or pull request on any branch | Spins up a `postgres:16` service container, installs [backend/requirements.txt](backend/requirements.txt) on Python 3.13 (matching the Docker images), then runs `pytest` against [backend/test_main.py](backend/test_main.py) from `backend/`, producing a JUnit XML report that is printed as a final step. |
...
| **Deploy** ([deploy.yml](.github/workflows/deploy.yml)) | Push to `main` | Runs with `environment: dev`; writes the `POSTGRES_USER`/`POSTGRES_DB`/`POSTGRES_PASSWORD` values into `.env` at the repo root, then runs `docker compose pull` and `docker compose up -d` to redeploy with the latest published images; requires nginx TLS files to be provisioned on the runner/host |
...
└── requirements.txt         # production + test deps (pytest, httpx) in one pinned, hash-locked file
frontend/

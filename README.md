...
|---|---|
| **REST API (FastAPI)** | [backend/main.py](backend/main.py) — `POST/GET/DELETE /people` endpoints, async handlers, auto-generated OpenAPI/Swagger docs |
| **Data validation (Pydantic)** | [backend/models.py](backend/models.py) — `PersonIn`/`PersonOut`/`DeleteRequest`, field validators (non-empty names, no digits, birth date not in the future, swag level range, min password length, password must contain a letter and a number), automatic `422` on invalid input |
...
| **Frontend UI** | [frontend/frontend.py](frontend/frontend.py) — Streamlit form + table, talks to the backend only via REST (`API_URL`) |
| **Password handling** | Passwords are hashed with PBKDF2-HMAC-SHA256 + per-user salt in [backend/main.py](backend/main.py); the plain password is never stored or returned |
| **Persistent storage (PostgreSQL)** | [backend/main.py](backend/main.py) — connects to PostgreSQL via `POSTGRES_*` env vars; data persists in a named Docker volume (Compose) or a `StatefulSet`'s `PersistentVolumeClaim` (Kubernetes) |
| **Containerization (Docker)** | Independent multi-stage [backend/Dockerfile](backend/Dockerfile) and [frontend/Dockerfile](frontend/Dockerfile), orchestrated by [docker-compose.yml](docker-compose.yml) |
| **Health checks / startup ordering** | `GET /health` on the backend and Streamlit's built-in `/_stcore/health`, wired into `HEALTHCHECK` in both Dockerfiles and `healthcheck:` + `depends_on: condition: service_healthy` in [docker-compose.yml](docker-compose.yml) (see [Health checks](#health-checks)) |
| **Reverse proxy + TLS** | `nginx` service in [docker-compose.yml](docker-compose.yml) using [nginx/nginx.conf](nginx/nginx.conf) — single public entry point on `80`/`443`, HTTPS via a self-signed certificate (see [Reverse proxy and TLS (nginx)](#reverse-proxy-and-tls-nginx)) |
...
| **Container orchestration (Kubernetes)** | [k8s](k8s) — `Deployment`/`Service`/`ConfigMap` per app service, a `StatefulSet` + `PersistentVolumeClaim` for PostgreSQL, `Secret`s for credentials/TLS, targeting Docker Desktop's built-in cluster (see [Kubernetes (Docker Desktop)](#kubernetes-docker-desktop)) |
...
- **nginx** ([nginx](nginx)) — reverse proxy and TLS termination point. It's the only service with ports published to the host (`80`/`443`); it forwards `/` to the frontend and `/api/` to the backend over the internal Docker network. See [Reverse proxy and TLS (nginx)](#reverse-proxy-and-tls-nginx).
- **backend** ([backend](backend)) — stores people's data in PostgreSQL, connecting via `POSTGRES_HOST`/`POSTGRES_PORT`/`POSTGRES_DB`/`POSTGRES_USER`/`POSTGRES_PASSWORD` env vars. Postgres itself runs as its own service/Pod (`postgres` in Compose, a `StatefulSet` in Kubernetes), persisting data in a named volume or `PersistentVolumeClaim` respectively — see [Docker / Docker Compose](#docker--docker-compose) and [Kubernetes (Docker Desktop)](#kubernetes-docker-desktop).
- **frontend** ([frontend](frontend)) — Streamlit UI, communicates with the backend exclusively through the REST API (over the internal Docker network, not through nginx).
...
|---|---|---|
| Backend HTTP | **FastAPI** + **Uvicorn** | [backend/main.py](backend/main.py) |
| Validation | **Pydantic** | [backend/models.py](backend/models.py) |
| Storage | **PostgreSQL** (`psycopg2`) | [backend/main.py](backend/main.py) |
| Frontend | **Streamlit** + **requests** | [frontend/frontend.py](frontend/frontend.py) |
| Containerization | **Docker** + **Docker Compose** | [backend/Dockerfile](backend/Dockerfile), [frontend/Dockerfile](frontend/Dockerfile), [docker-compose.yml](docker-compose.yml) |
...
|---|---|---|---|
| `API_URL` | frontend | `http://127.0.0.1:8000` | Backend address the frontend sends HTTP requests to. Set to `http://backend:8000` in [docker-compose.yml](docker-compose.yml) so the frontend can reach the backend by its service name on the Docker network. |
| `POSTGRES_HOST` | backend | None; required | Hostname of the PostgreSQL server. Set to `postgres` in both [docker-compose.yml](docker-compose.yml) and the Kubernetes manifests — the DNS name of the `postgres` service on the same network. |
...
| `POSTGRES_PASSWORD` | backend | None; required | Password for `POSTGRES_USER`. Sourced from a git-ignored `.env` file locally, a `dev` GitHub Environment secret in CI (Compose), or a Kubernetes `Secret` named `postgres-credentials` (Kubernetes) — never committed. |

The backend's host and port are set directly in [backend/Dockerfile](backend/Dockerfile) (`uvicorn main:app --host 0.0.0.0 --port 8000`).
...
Create a git-ignored `.env` file so Compose can resolve `${POSTGRES_USER}`/`${POSTGRES_PASSWORD}`/`${POSTGRES_DB}` (this mirrors the same-named variables/secret stored in the `dev` GitHub Environment, used the same way by [deploy.yml](.github/workflows/deploy.yml) in CI):
...
As an alternative to Docker Compose, the same services can be run on Kubernetes — manifests live in [k8s](k8s), targeting the single-node cluster built into Docker Desktop (Settings → Kubernetes → Enable Kubernetes).

| File | Kind | Purpose |
|---|---|---|
| [backend-deployment.yaml](k8s/backend/backend-deployment.yaml) | `Deployment` | Runs the backend image; `POSTGRES_*` env vars from `backend-config`/`postgres-credentials`; readiness/liveness probes on `/health` |
| [backend-service.yaml](k8s/backend/backend-service.yaml) | `Service` (`ClusterIP`) | Internal DNS name `backend:8000` |
| [backend-configmap.yaml](k8s/backend/backend-configmap.yaml) | `ConfigMap` | Non-secret backend config: `POSTGRES_HOST`, `POSTGRES_PORT`, `POSTGRES_DB` |
| [backend-pvc.yaml](k8s/backend/backend-pvc.yaml) | `PersistentVolumeClaim` | Left over from an earlier SQLite-based setup; no longer mounted by `backend-deployment.yaml` now that Postgres owns storage — kept only for reference |
| [frontend-deployment.yaml](k8s/frontend/frontend-deployment.yaml) | `Deployment` | Runs the frontend image; `API_URL` from `frontend-config`; probes on `/_stcore/health` |
| [frontend-service.yaml](k8s/frontend/frontend-service.yaml) | `Service` (`ClusterIP`) | Internal DNS name `frontend:8501` |
| [frontend-configmap.yaml](k8s/frontend/frontend-configmap.yaml) | `ConfigMap` | `API_URL: http://backend:8000` |
| [postgres-statefulset.yaml](k8s/postgres/postgres-statefulset.yaml) | `StatefulSet` | Runs `postgres:16` with a dedicated `PersistentVolumeClaim` per replica via `volumeClaimTemplates`; credentials from `postgres-credentials` |
| [postgres-service.yaml](k8s/postgres/postgres-service.yaml) | `Service` (headless, `clusterIP: None`) | Stable per-Pod DNS names required by the `StatefulSet` |
| [nginx-deployment.yaml](k8s/nginx/nginx-deployment.yaml) | `Deployment` | Runs `nginx:latest`, mounting `nginx-config` and the `nginx-tls` secret |
| [nginx-service.yaml](k8s/nginx/nginx-service.yaml) | `Service` (`LoadBalancer`) | Publishes `80`/`443` to the host — Docker Desktop binds this to `localhost` |
| [nginx-configmap.yaml](k8s/nginx/nginx-configmap.yaml) | `ConfigMap` | Same reverse-proxy/TLS logic as [nginx/nginx.conf](nginx/nginx.conf), adapted to reference Kubernetes `Service` DNS names/ports |
...
kubectl create secret tls nginx-tls `
  --cert=nginx/certs/selfsigned.crt `
  --key=nginx/certs/selfsigned.key
```

Apply everything (this order lets PVCs/Secrets exist before the Pods that reference them, though `kubectl apply -f k8s/ -R` for the whole folder at once also works — Kubernetes retries scheduling until dependencies appear):
...
|---|---|---|
| backend | `GET /health` → `{"status": "ok"}` | [backend/main.py](backend/main.py), [backend/Dockerfile](backend/Dockerfile) |
| frontend | `GET /_stcore/health` (built into Streamlit) | [frontend/Dockerfile](frontend/Dockerfile) |

Both Dockerfiles probe their endpoint with `python -c "import urllib.request; ..."` instead of `curl`/`wget`, since the `dhi.io/python` base image doesn't include either. [docker-compose.yml](docker-compose.yml) redefines the same checks under `healthcheck:` on each service (letting interval/timeout be tuned without a rebuild), and `frontend` declares `depends_on: backend: condition: service_healthy` so Compose won't start the frontend container until the backend's healthcheck reports healthy — avoiding the connection-refused race where the frontend's first requests hit a backend that isn't listening yet.
...
The `nginx` service in [docker-compose.yml](docker-compose.yml) sits in front of both application services and is the only container with ports published to the host (`80`/`443`). Its config, [nginx/nginx.conf](nginx/nginx.conf), does two things:
...
- **TLS termination** — port 80 redirects to port 443; port 443 serves HTTPS using a certificate/key expected at `nginx/certs/selfsigned.crt` / `selfsigned.key` (git-ignored, generated locally — see [Running locally](#running-locally)).
...
`pytest` and `httpx` are tracked alongside production dependencies in [backend/requirements/requirements.in](backend/requirements/requirements.in) / [backend/requirements/requirements.txt](backend/requirements/requirements.txt) — a single hash-locked requirements file shared by the Docker build and CI.
...
|---|---|---|
| **Auto Tests** ([auto_tests.yml](.github/workflows/auto_tests.yml)) | Push or pull request on any branch | Spins up a `postgres:16` service container, installs [backend/requirements/requirements.txt](backend/requirements/requirements.txt) on Python 3.13, then runs `pytest` against [backend/test_main.py](backend/test_main.py) from `backend/`, producing a JUnit XML report that is printed as a final step. |
...
| **Deploy** ([deploy.yml](.github/workflows/deploy.yml)) | Push to `main` | Runs with `environment: dev`; writes the `POSTGRES_USER`/`POSTGRES_DB`/`POSTGRES_PASSWORD` values into `.env`, then runs `docker compose pull` and `docker compose up -d` to redeploy with the latest published images; requires nginx TLS files to be provisioned on the runner/host |

Dependency updates are automated by [Dependabot](.github/dependabot.yml), which opens pull requests on a daily schedule for: the backend's and frontend's `pip` dependencies, the base images referenced by both [Dockerfiles](backend/Dockerfile) (authenticated against the `dhi.io` registry via Dependabot secrets), and the GitHub Actions used in the workflows above.
...
├── .dockerignore           # excludes tests, caches from the build context
...
└── requirements/
    ├── requirements.in     # top-level deps (fastapi, pydantic, uvicorn, psycopg2-binary, httpx, pytest)
    └── requirements.txt    # hash-locked, pip-compile'd from requirements.in
frontend/
├── Dockerfile              # multi-stage Streamlit image (dhi.io/python base), HEALTHCHECK on /_stcore/health
├── .dockerignore           # excludes caches from the build context
